...
void Game::CreateNewPiece()
{
	...
	if(mPiece == 1 && mRotation == 0) {
		mPosX		= (rand() % (BOARD_WIDTH-3))+ 1 + mPieces->GetXInitialPosition (mPiece, mRotation);
	}
	else if(mPiece == 1 && mRotation == 2) {
		mPosX		= (rand() % (BOARD_WIDTH - 3))+ 2 + mPieces->GetXInitialPosition (mPiece, mRotation);
	}
	else {
		mPosX		= (rand() % (BOARD_WIDTH-2)) + 1 + mPieces->GetXInitialPosition (mPiece, mRotation);
	}
	//mPosX 			= (BOARD_WIDTH / 2) + mPieces->GetXInitialPosition (mPiece, mRotation);
	mPosY 			= mPieces->GetYInitialPosition (mPiece, mRotation);
...
void MonteCarlo()
{
	...
	// calls draw piece with monte carlo generated values
}
